...
follow_feed(Name, Url, Workers) ->
    supervisor:start_child(couchfeedreader_sup, 
			   {Name, {feed_sup, start_link, [Name, Url, Workers]},
			    temporary, 10000, supervisor, [feed_sup]}).
...
list_feeds() -> 
    Specs =
	supervisor:which_children(couchfeedreader_sup),
    lists:map(fun({Id,Pid,_,_}) -> {Id, Pid} end, Specs).

feed_info(Name) ->
    {ok, Spec} =
	supervisor:get_childspec(couchfeedreader_sup, Name), 
    Args =
	element(3,maps:get(start, Spec)),
